...
class LLMJudge:
    ...
    def score_response(
        ...
        prompt = (
            "Evaluate the response using every rubric criterion. Return only "
            "a JSON object mapping each criterion to a score from 0 to 1.\n\n"
            f"Question:\n{question}\n\n"
            f"Response:\n{answer}\n\n"
            f"Rubric:\n{rubric_text}"
        )
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        position_pairs = list(zip(response_averages[::2], response_averages[1::2]))
...
class FailureAnalyzer:
    ...
    def generate_improvement_log(self, failures: list, suggestions: list[str]) -> str:
        ...
        for index, failure in enumerate(failures, start=1):
            suggestion = (
                ...
                else "Review this failure and define a targeted remediation"
            )
            values = (
                f"F{index:03d}",
    ...
    def generate_improvement_suggestions(
        ...
        actions = {
            ...
            "irrelevant": (
                "Clarify the system prompt and add intent-routing examples for ambiguous questions"
            ),
            "incomplete": (
                "Improve retrieval coverage and require answers to address every requested item"
            ),
            "off_topic": (
                "Add an intent classifier and explicit in-scope response instructions"
            ),
            "refusal": (
                "Calibrate guardrails so valid Student Services questions are answered"
            ),
            "unknown": (
                "Review uncategorized traces and add a deterministic failure label"
            ),
        }
        suggestions = [
        ...
        fallback_actions = [
            "Increase chunk overlap and tune top-k to reduce missing evidence",
            "Add regression cases for every observed failure before changing the pipeline",
            "Track answer and retrieval metrics separately after each prompt or retriever change",
        ]
